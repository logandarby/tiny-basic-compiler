...
Test(token_array, multiple_pushes_within_capacity) {
  ...
  for (uint32_t i = 0; i < 100; i++) {
    cr_assert_eq(token_array_at(ta, i).type, TOKEN_PLUS,
                 "Element %" PRIu32 " should be TOKEN_PLUS", i);
  }
...
Test(token_array, resize_at_capacity_boundary) {
  ...
  for (uint32_t i = 0; i < 512; i++) {
    cr_assert_eq(token_array_at(ta, i).type, TOKEN_MINUS,
                 "Element %" PRIu32 " should be TOKEN_MINUS", i);
  }
...
Test(token_array, multiple_resizes) {
  ...
  const uint32_t target_size = 2048 + 10; // Force multiple resizes
  ...
  for (uint32_t i = 0; i < target_size; i++) {
    ...
    cr_assert_eq(token_array_at(ta, i).type, expected,
                 "Element %" PRIu32 " should be correct after resizes", i);
  }
...
Test(token_array, resize_sequence_validation) {
  ...
  uint32_t expected_capacities[] = {512, 1024, 2048, 4096, 8192};
  uint32_t capacity_index = 0;

  cr_assert_eq(token_array_capacity(ta), expected_capacities[capacity_index],
               "Initial capacity should be %" PRIu32 "",
    ...
    if (i == expected_capacities[capacity_index]) {
      ...
      if (capacity_index <
        ...
        cr_assert_eq(token_array_capacity(ta),
                     expected_capacities[capacity_index],
                     "Capacity should be %" PRIu32 " after resize",
                     expected_capacities[capacity_index]);
      }
    }
  }
...
Test(token_array, large_scale_operations) {
  ...
  cr_assert_eq(token_array_length(ta), large_size,
               "Length should be %" PRIu32 "", large_size);
  ...
  for (uint32_t i = 0; i < large_size; i++) {
    ...
    cr_assert_eq(token_array_at(ta, i).type, expected,
                 "Element %" PRIu32 " should be correct", i);
  }
...
Test(token_array, alternating_token_stress) {
  ...
  cr_assert_eq(token_array_length(ta), iterations * 5,
               "Length should be %" PRIu32 "", iterations * 5);

  // Verify the alternating pattern
  for (uint32_t i = 0; i < iterations; i++) {
    cr_assert_eq(token_array_at(ta, i * 5 + 0).type, TOKEN_PLUS,
                 "Pattern verification failed at %" PRIu32 "", i);
...
Test(token_array, immediate_resize_trigger) {
  ...
  uint32_t capacity_before = token_array_capacity(ta);
  ...
  uint32_t capacity_after = token_array_capacity(ta);
...
Test(token_array, single_element_to_resize) {
  ...
  for (uint32_t i = 0; i <= 512; i++) { // 513 elements total
...
Test(token_array, exact_power_of_two_boundaries) {
  ...
  uint32_t boundaries[] = {512, 1024, 2048};

  for (uint32_t b = 0; b < sizeof(boundaries) / sizeof(boundaries[0]); b++) {
    // Clear and test each boundary
    token_array_destroy(&ta);
    ta = token_array_init();

    uint32_t target = boundaries[b];
    ...
    cr_assert_eq(token_array_length(ta), target,
                 "Should have exactly %" PRIu32 " elements", target);
    cr_assert_eq(token_array_capacity(ta), target,
                 "Capacity should be exactly %" PRIu32 "", target);

    // One more should trigger resize
    token_array_push_simple(ta, TOKEN_ENDWHILE, fl);
    cr_assert_eq(token_array_capacity(ta), target * 2,
                 "Should double to %" PRIu32 "", target * 2);
  }
...
Test(token_array, data_integrity_across_resizes) {
  ...
  const uint32_t pattern_size = 1000;
  ...
  for (uint32_t i = 0; i < pattern_size; i++) {
    cr_assert_eq(
        token_array_at(ta, i).type, pattern[i],
        "Pattern element %" PRIu32 " should be preserved across resizes", i);
  }

  // Verify the added elements
  for (uint32_t i = pattern_size; i < pattern_size + 2000; i++) {
    cr_assert_eq(token_array_at(ta, i).type, TOKEN_UNKNOWN,
                 "Added element %" PRIu32 " should be TOKEN_UNKNOWN", i);
  }
...
Test(token_array, capacity_never_shrinks) {
  ...
  uint32_t initial_capacity = token_array_capacity(ta);
  ...
  uint32_t after_resize = token_array_capacity(ta);
...
Test(token_array, amortized_growth_pattern) {
  ...
  uint32_t previous_capacity = token_array_capacity(ta);
  ...
  for (uint32_t i = 0; i < 5000; i++) {
    ...
    uint32_t current_capacity = token_array_capacity(ta);
